...
function downloadRepository() {
  ...
  if (
      document.getElementById("repoSave").value != null ||
      document.getElementById("repoSave").value != ""
  ) {
...
function downloadFunc(cloneURL, fullLocalPath) {
  ...
  let repository = Git.Clone.clone(cloneURL, fullLocalPath, options).then(
      function(repository) {
        ...
        updateModalText(
            "Clone Successful, repository saved under: " + fullLocalPath
        );
...
function openRepository() {
  ...
  if (
      document.getElementById("dirPickerOpenLocal").value === previousOpen &&
  ...
  // Full path is determined by either handwritten directory or selected by file browser
  if (
      document.getElementById("repoOpen").value == null ||
      document.getElementById("repoOpen").value == ""
  ) {
    let localPath = document.getElementById("dirPickerOpenLocal").files[0]
        .webkitRelativePath;
    let fullLocalPath = document.getElementById("dirPickerOpenLocal").files[0]
        .path;
  ...
  Git.Repository.open(fullLocalPath).then(
      function(repository) {
        ...
        if (readFile.exists(repoFullPath + "/.git/config")) {
          let gitConfigFileText = readFile.read(
              repoFullPath + "/.git/config",
              null
          );
          let searchString = '[remote "origin"]';

          gitConfigFileText = gitConfigFileText.substr(
              gitConfigFileText.indexOf(searchString) + searchString.length,
              gitConfigFileText.length
          );
          gitConfigFileText = gitConfigFileText.substr(
              0,
              gitConfigFileText.indexOf(".git")
          );
          ...
          if (gitConfigFileSubstrings[0].indexOf("@") != -1) {
            gitConfigFileSubstrings[0] = gitConfigFileSubstrings[0].substring(
                gitConfigFileSubstrings[0].indexOf(":") + 1
            );
          }

          let repoOwner =
              gitConfigFileSubstrings[gitConfigFileSubstrings.length - 2];
            ...
            $.ajax({
              ...
              beforeSend: function(xhr) {
                xhr.setRequestHeader(
                    "Authorization",
                    make_base_auth(getUsername(), getPassword())
                );
              },
              headers: {
              ...
              error(xhr, status, error) {
                console.log(
                    "The XML Http Request of the GitHub API call is: ",
                    xhr
                );
      ...
      function(err) {
        updateModalText(
            "No repository found. Select a folder with a repository."
        );
...
function createLocalRepository() {
  ...
  if (
      document.getElementById("repoCreate").value == null ||
      document.getElementById("repoCreate").value == ""
  ) {
    document.getElementById("dirPickerCreateLocal").click();
    let localPath = document.getElementById("dirPickerCreateLocal").files[0]
        .webkitRelativePath;
    let fullLocalPath = document.getElementById("dirPickerCreateLocal").files[0]
        .path;
    ...
    if (!require("path").isAbsolute(localPath)) {
      updateModalText(
          "The filepath is not valid. For OSX and Ubuntu the filepath should start with /, for Windows C:\\\\"
      );
      return;
    } else {
      if (checkFile.existsSync(localPath)) {
  ...
  if (checkFile.existsSync(require("path").join(fullLocalPath, ".git"))) {
    ...
    updateModalText(
        "This folder is already a git repository. Please try to open it instead."
    );
  } else {
    displayModal(
        "creating repository at " + require("path").join(fullLocalPath, ".git")
    );
    Git.Repository.init(fullLocalPath, 0).then(
        function(repository) {
          repoFullPath = fullLocalPath;
          repoLocalPath = localPath;
          refreshAll(repository);
  ...
  repository
      ...
      .then(function(refList) {
        ...
        for (let i = 0; i < refList.length; i++) {
          ...
          //get simplified name
          let refName = refList[i].name().split("/")[
          refList[i].name().split("/").length - 1
              ];

          Git.Reference.nameToId(repository, refList[i].name()).then(
              function(oid) {
                // Use oid
                if (refList[i].isRemote()) {
                  ...
                  console.log(
                      refName + ": adding branch to end of " + oid.tostrS()
                  );
              ...
              function(err) {
                console.log(
                    "repo.ts, line 273, could not find referenced branch" + err
                );
              }
          );
      ...
      .then(
          function() {
            ...
            if (repoLocalPath.length > 20) {
              ...
              repoLocalPath =
                  "..." + repoLocalPath.slice(breakStringFrom, repoLocalPath.length);
            }
          ...
          function(err) {
            ...
            console.log(
                "No branches found. Setting default label values to master"
            );
  ...
  Git.Repository.open(repoFullPath)
      ...
      .then(function(ref) {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (
              bp[1] !== "remotes" &&
              bp[bp.length - 1] !== name[name.length - 1]
          ) {
            displayBranch(
                bp[bp.length - 1],
                "merge-dropdown",
                "mergeLocalBranches(this)"
            );
          }
        }
      });
  ...
  if (id == "branch-item-list") {
    ...
    Git.Repository.open(repoFullPath).then(function(repo) {
      Git.Reference.list(repo).then(function(array) {
        if (array.includes("refs/remotes/origin/" + name)) {
          a.innerHTML +=
              "<img src='./assets/remote-branch.png' width='20' height='20' align='right' title='Remote'>";
          isRemote = 1;
        }
      });
    });
    ...
    Git.Repository.open(repoFullPath).then(function(repo) {
      repo.getBranch(name).then(function() {
        a.innerHTML +=
            "<img src='./assets/local-branch.png' width='20' height='20' align='right' title='Local'>";
        isLocal = 1;
      });
    });
...
function checkoutLocalBranch(element) {
  ...
  Git.Repository.open(repoFullPath).then(function(repo) {
    ...
    repo.checkoutBranch("refs/heads/" + bn).then(
        function() {
          refreshAll(repo);
        },
        function(err) {
  ...
  Git.Repository.open(repoFullPath)
      ...
      .then(
          function(code) {
            ...
            repos.mergeBranches(bn, "origin/" + bn).then(function() {
              document.getElementById("spinner").style.display = "block";
              refreshAll(repos);
...
function updateLocalPath() {
  ...
  if (splitText[splitText.length - 1] == "git") {
    ...
    fullLocalPath = require("path").join(
        __dirname,
        splitText[splitText.length - 2]
    );
...
function chooseLocalPath() {
  if (
      document.getElementById("repoClone").value == null ||
      document.getElementById("repoClone").value == ""
  ) {
    ...
    localPath = document.getElementById("dirPickerSaveNew").files[0]
        .webkitRelativePath;
    fullLocalPath = document.getElementById("dirPickerSaveNew").files[0].path;
